...
d3.csv("densities/data/sim_12_5_13_for_d3.csv", function(input) {
  ...
  for(var g = 0; g < estimators.length; g++)
  {
      ...
      data = _.toArray(data_filtered.filter(Compare(estimators[g], 'estimator'))[0]).slice(4,304).map(Number);
  ...
  for (var i=0; i<estimators.length; i++) {
	  ...
	  data = _.toArray(data_filtered.filter(Compare(estimators[i], 'estimator'))[0]).slice(7,307).map(Number);
...
unscaley.range(ydomain);
  ...
  d3.select("#sliderIStext").text(d3.min(values_is));
  d3.select('#sliderIS')
        .call( d3.slider()
                        //.axis(true)
                        ...
                        //.orientation("vertical")
                        .step(0.2)
                        .value(d3.min(values_is))
                        .on("slide", function(evt, value) {
      				d3.select('#sliderIStext').text(d3.round(Math.floor(100*value)/100, 1));
				is = d3.round(Math.floor(100*value)/100, 1);
  curris = is;
  data_filtered = input.filter(Compare(corux, 'EFFUX')).filter(Compare(coruy, 'EFFUY')).filter(Compare(is, 'IS'));
  var real_corux = d3.round(_.toArray(data_filtered.filter(Compare(estimators[0], 'estimator'))[0])[1], 4);
  d3.select("#sliderCORUXtext").text(real_corux);
